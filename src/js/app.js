...
App = {
  web3Provider: null,
  contracts: {},
  account: '0x0', // Initialize account as a string
    ...
    if (window.ethereum) {
      try {
        ...
        if (error.code === 4001) {
          ...
          console.error('Error occurred while requesting MetaMask accounts:', error);
        }
      }
    } else {
  ...
  castVote: function (id) {
    ...
    if (window.ethereum) {
      window.ethereum.request({ method: 'eth_requestAccounts' }).then(function (accounts) {
        ...
        // Now that we have the selected account, we can call the vote function without specifying the 'from' address
        App.contracts.Contest.deployed().then(function (instance) {
  ...
  changeState: function () {
    ...
    App.contracts.Contest.deployed().then(function (instance) {
      // Specify the transaction options including the 'from' address
      var txOptions = {
        from: App.account
      };
      return instance.changeState(phaseEnum, txOptions); // Pass the transaction options to the contract function
    }).then(function (result) {
      $("#content").hide();
      $("#loader").show();
    }).catch(function (err) {
